...
def check_ollama():
    try:
        ...
        print(f"   G¥î Ollama is not reachable: {e}")
        return False


def warmup_ollama():
    print(f"\n=ƒöÑ Warming up Ollama model ({OLLAMA_MODEL})...")
    try:
        ...
        response.raise_for_status()
        print("   G£à Ollama model is ready")
        return True
    except Exception as e:
        print(f"   GÜán+Å  Ollama warm-up failed: {e}")
        return False
...
def query_ollama_structured(prompt, timeout=OLLAMA_TIMEOUT, retries=OLLAMA_RETRIES):
    ...
    for attempt in range(retries + 1):
        try:
            ...
            print(f"   GÜán+Å  Empty or non-structured response on attempt {attempt + 1}/{retries + 1}")
...
# GöÇGöÇ PROMPTING GöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇ
...
# GöÇGöÇ TOPIC CLASSIFICATION GöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇ
...
def classify_topics(df):
    print(f"\n=ƒñû Classifying topics with Ollama ({OLLAMA_MODEL})...")
...
# GöÇGöÇ EMBEDDINGS FOR RECOMMENDATION GöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇ
def embed_passages(df):
    print(f"\n=ƒöó Embedding passages with '{EMBED_MODEL}' (runs locally)...")
    ...
    print(f"   G£à Embeddings shape: {embeddings.shape}")
    return embeddings, model


# GöÇGöÇ BUILD OUTPUT GöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇ
...
# GöÇGöÇ RECOMMENDATION GöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇ
...
def demo_recommendations(embed_model, df_out, embeddings):
    print("\n=ƒÄô Demo: Student Recommendations")
    print("GöÇ" * 60)

    demo_students = [
        {
            "name": "Student A",
            "interest": "Jeg liker natur, dyr og milj++",
            "preferred_topics": ["Natur", "Dyr", "Milj++"]
        },
        {
            "name": "Student B",
    ...
    for student in demo_students:
        print(f'\n=ƒæñ {student["name"]}: "{student["interest"]}"')
        ...
        for _, row in recs.iterrows():
            print(
                f"   GåÆ [{row['serialNumber']}] {row['title']} | "
                f"{row['main_topic']} | sim={row['similarity']:.3f} | "
                f"conf={row['topic_confidence']:.2f}"
            )


# GöÇGöÇ MAIN GöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇ
def main():
    print("=" * 55)
    ...
    print(f"\n=ƒÆ+ Saved categorized passages GåÆ {out_path}")
