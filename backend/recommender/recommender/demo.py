"""
Demo — Simulated Session Walk-through
======================================
...
def simulate_feedback(text_difficulty: float, true_level: float) -> dict:
    ...
    # Round to nearest third (3 questions → 0/3, 1/3, 2/3, 3/3)
    comp = round(comp * 3) / 3
...
def run_session(corpus, interests, true_level, label, n_rounds=6):
    ...
    print(f"  Sant nivå   : {true_level:.1f} (ukjent for systemet)")
    ...
    for rnd in range(1, n_rounds + 1):
        ...
        if random.random() < 0.2 and rnd > 1:
            print(f"    ↻ Studenten trykker oppfrisk")
...
def main():
    print_header("PERSONALISERT TEKSTANBEFALING — DEMO", width=65)
    print("  Simulerer fullstendige økter med syntetiske elevprofiler")
    print("  Viser hvordan anbefalinger tilpasses gjennom økten")
    ...
    print(f"\n  Korpus: {stats['total_texts']} tekster "
          f"({stats['reliable_texts']} pålitelige)")
    ...
    run_session(
        ...
        true_level=2.2,
        label="Elev A — Lett leser, naturinteressert",
        n_rounds=6,
    )
    ...
    run_session(
        ...
        true_level=4.0,
        label="Elev B — Avansert leser, STEM-interessert",
        n_rounds=6,
    )
    ...
    run_session(
        ...
        true_level=3.2,
        label="Elev C — Middels leser, bredt interessert",
        n_rounds=6,
    )
