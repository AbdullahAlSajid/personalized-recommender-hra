...
1. Difficulty MAE      — how well recommendations match the student's level
2. Topic recall        — fraction of interests covered across the session
3. Intra-slate diversity — topic dissimilarity within each 2-text slate
4. Corpus coverage     — what % of corpus texts are reachable
5. Comprehension proxy — simulated comprehension vs difficulty gap
6. Adaptive convergence — how quickly estimated_level approaches true_level
...
def evaluate(
    ...
    if verbose:
        ...
        print(f"\n  ── Difficulty MAE by true reading level ──")
        m_copy = m.copy()
        ...
        print(f"  MAE reduction (R1 → R2+)  : {improvement:.3f}  "
