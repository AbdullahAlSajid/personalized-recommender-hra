...
Total comparisons: 157 × 156 / 2 = 12,246 API calls
...
Inputs:
    ...
    difficulty_scores_v2.csv       — anchor-based scores from Stage 1

Outputs:
    pairwise_checkpoint.json       — auto-saved progress after every pair
    pairwise_results_full.csv      — all 12,246 comparison results
    pairwise_scores.csv            — win counts + normalized 1.0-5.0 per text
    final_difficulty_scores.csv    — combined anchor + pairwise scores
    validation_metrics.json        — Kendall's tau between the two methods
"""
...
MAX_RETRIES       = 3
RETRY_DELAY       = 5
SLEEP_BETWEEN     = 0.1     # paid tier — 1K RPM
...
Consider these dimensions in order of importance:
  1. Vocabulary difficulty    — domain-specific, technical, or uncommon words
  2. Background knowledge     — prior knowledge a 9-11-year-old needs
  3. Abstractness             — concrete vs abstract ideas and concepts
  4. Sentence complexity      — length and syntactic depth
  5. Inferential demand       — how much the reader must infer
...
def run_pairwise(client: Groq, texts: list) -> pd.DataFrame:
    ...
    print(f"\n  Comparisons complete → {RESULTS_CSV}")
    return results_df


# ── Stage 2: Compute pairwise scores ─────────────────────────────────────────
...
def compute_pairwise_scores(results_df: pd.DataFrame, texts: list) -> pd.DataFrame:
    ...
    print(f"\n  Saved → {PAIRWISE_SCORES}")
    return scores_df


# ── Stage 3: Combine anchor + pairwise ───────────────────────────────────────
...
def compute_kendall(merged: pd.DataFrame) -> dict:
    ...
    print(f"\n{'═'*55}")
    print(f"  VALIDATION — KENDALL'S τ")
    print(f"{'═'*55}")
    ...
    print(f"\n  τ ≥ 0.7 = strong agreement — both methods consistent")
    print(f"  τ ≥ 0.5 = moderate agreement — acceptable for thesis")
    print(f"  τ < 0.5 = weak agreement — investigate discrepancies")
