...
# Ollama — swap model if llama3 still collapses:
#   "mistral"      ← great structured output
#   "gemma2:9b"   ← strong instruction following
#   "llama3.1:8b" ← slightly better than llama3
OLLAMA_MODEL = "llama3"
...
# ── Anchor examples ───────────────────────
ANCHORS = {
    1: "Jeg har en katt. Den er svart. Katten heter Leo. Leo liker å sove.",
    2: "Vi har mange fag på skolen. Jeg liker best norsk og gym. Læreren vår er veldig snill.",
    3: "Mange barn i Norge bruker mye tid foran skjermen. Foreldre er bekymret for dette.",
    4: "Klimaendringer har ført til hyppigere ekstremvær og stigende havnivåer de siste tiårene.",
    5: "Kvantemekanikken utfordrer vår intuitive forståelse av kausalitet og lokalitet i fysikkens lover.",
    6: "Den fenomenologiske tradisjonen insisterer på at bevisstheten alltid er intensjonalt rettet mot verden.",
}
# ─────────────────────────────────────────
...
def call_ollama(prompt, num_predict=200, timeout=OLLAMA_TIMEOUT, retries=OLLAMA_RETRIES):
    """Raw Ollama call — returns the response string or None."""
    payload = {
        "model": OLLAMA_MODEL,
        "prompt": prompt,
        "stream": False,
...
def score_single_passage(title, body):
    ...
    for run in range(SCORE_REPEATS):
        ...
        if not _debug_printed:
            print("\n" + "─" * 55)
            print("DEBUG — Step 1 (analysis) raw output:")
        ...
        if not _debug_printed:
            print("DEBUG — Step 2 (score) raw output:")
            print(repr(score_raw[:300]))
            print("─" * 55 + "\n")
            _debug_printed = True

        # Parse — three layers of fallback
        ...
        if parsed is None:
            print(f"   ⚠️  Could not parse score (run {run + 1}): {repr((score_raw or '')[:120])}")
            scores.append(3.0)
...
def _save_checkpoint(df, results, up_to):
    ...
    print(f"   💾 Checkpoint saved ({up_to} passages)")
...
def print_distribution(df_out):
    ...
    if not high_var.empty:
        print(f"\n⚠️  {len(high_var)} high-variance passage(s) (std ≥ 1.0) — consider manual review:")
    ...
    print(f"   Range : {df_out['difficulty_score'].min():.0f} – {df_out['difficulty_score'].max():.0f}")
